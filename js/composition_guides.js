...
const GuideDrawer = {
    ...
    drawGoldenTriangles(ctx, p) {
        ...
        function getLineEndpoints(point, m) {
            ...
            let x_at_y0 = (0 - point.y) / m + point.x;
            ...
            return points.filter((v,i,a)=>a.findIndex(t=>(Math.abs(t.x-v.x) < 1e-9 && Math.abs(t.y-v.y) < 1e-9))===i);
        }
        ...
        if (p.mode.startsWith("Both") || p.mode.startsWith("Set 1")) {
            ...
            const phi_points1 = [
                { x: w / (PHI + 1), y: m1 * (w / (PHI + 1)) },
                { x: w * PHI / (PHI + 1), y: m1 * (w * PHI / (PHI + 1)) },
                { x: (h / (PHI + 1)) / m1, y: h / (PHI + 1) },
                { x: (h * PHI / (PHI + 1)) / m1, y: h * PHI / (PHI + 1) }
            ];
            for(const pt of phi_points1) {
        ...
        if (p.mode.startsWith("Both") || p.mode.startsWith("Set 2")) {
            ...
            const phi_points2 = [
                { x: w / (PHI + 1), y: m2 * (w / (PHI + 1) - w) },
                { x: w * PHI / (PHI + 1), y: m2 * (w * PHI / (PHI + 1) - w) },
                { x: (h / (PHI + 1)) / m2 + w, y: h / (PHI + 1) },
                { x: (h * PHI / (PHI + 1)) / m2 + w, y: h * PHI / (PHI + 1) }
            ];
            for(const pt of phi_points2) {
